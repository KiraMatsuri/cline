...
- **悬浮窗在长页面中无法完整显示**（反馈弹窗 / 服务器设置 / 工具调用确认等全部 `ResponsiveModal` 场景）：覆盖层此前 `top:0; bottom:0` 铺满整个文档并以**文档中心**垂直对齐——LLM 设置页滚动到底部打开弹窗时，弹窗悬于视口上方的文档中部。改为**锁定可视视口**（`top = 当前滚动偏移`、`height = 可视高度`，scroll 监听实时跟随），弹窗始终在可见区域居中，等效全局顶层固定；弹窗高于视口时自动改为顶部对齐可滚动，不再裁切
...
- **教学限制模式**：LLM 设置「高级选项」新增开关，启用后编辑器内单次粘贴最多 5 行，超出拦截并提示（防整段拷贝；未启用零影响，仅影响编辑器内 Ctrl/⌘+V）
- 教学 LLM 设置新增 `pasteLimit` 字段持久化（本地 env + VS Code 配置，两入口保存行为一致）

### Changed

- **周数筛选器重构**：tab 栏「第 X 周」选择器去除白底与硬编码颜色 → 主题前景色细边框；交互从下拉选择改为 ▲▼ 箭头步进（18↔1 循环）+ 悬停滚轮调节 + 双击直接输入周数（非法值保持不变并提示）；连续调节防抖 500ms 只派发最终值；移至服务器设置按钮右侧

### Fixed

- **修复 vsix 安装版图标大面积异常**（发送按钮、MCP Servers、Auto-approve 展开箭头等显示为乱码箭头/方块）：codicon 字体此前从 `node_modules` 加载，而打包时被排除导致 404；现随 webview 构建产物分发（`webview-ui/build/assets/codicon.css|ttf`），112 处图标一次性恢复
- **服务器设置悬浮窗**改用 `ResponsiveModal`：宽度随侧栏拖拽自适应，不再固定 360~480px 溢出窄侧栏
- LLM 设置视图 HTML 补齐 `index.css`/`codicon.css` 样式引用（此前只加载 JS）
- 实验任务网络请求（获取/提交/附件下载）自动重试 4 次指数退避，缓解不稳定网络下间歇性 `ECONNRESET`
